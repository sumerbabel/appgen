...
export class MenuTree extends formModel implements TreeGeneric {
  public  _id: string;
  private _idParent: string;
  private _idSecurrityMenu:string;
  public  _isOpen: boolean;
  private _hasChildren: boolean;
  private _menu: MenuData;
  public  _children: MenuTree[] = [];
  ...
  set childrens(children: MenuTree[]) {
    this._children = children;
  }

  set childrenPush(menuNode: MenuTree) {
    this._children.push(menuNode);
  }

  get childrens(): MenuTree[] {
    return this._children;
  }

  set menu(menu: MenuData) {
    this._menu = menu;
  }
  ...
  private setIsOpenRecursiveAll(menuNode: MenuTree[], isOpen: boolean): void {
  ...
  static createMenuNode(object: Object): MenuTree {
    let isopen: boolean;
    ...
    let menuAction: MenuData;
    if (object['menu']) {
      menuAction = MenuData.createMenu(object['menu']);
    }

    let menuNode: MenuTree = new MenuTree(
      object['id'],
      object['id_parent'],
  ...
  static createMenuNodeRecursive(object: Object): MenuTree {
    let manuNodeInitial: MenuTree = this.createMenuNode(object);
  ...
  public deleteChildren(menuNode: MenuTree) {
  ...
  public addChildren(): void {
    let menuNode: MenuTree = MenuTree.createMenuNodeEmpty(
      this.menu.idSistem,
      this.menu.idModule
    );
  ...
  static createMenuNodeEmpty(idSistem: string, idModule): MenuTree {
    let menuAction: MenuData = MenuData.createMenuEmpty(idSistem, idModule);
    let menuNode: MenuTree = new MenuTree(
      MenuTree.generateUuid(),
      undefined,
      menuAction.id,
      false,
      false,
  ...
  private dataPersistAll(menuNode: MenuTree[], arrayDataPersist: Array<any>) {
    menuNode.forEach((item) => {
