...
@Component({
  selector: 'app-update-table-item',
  templateUrl: './update-table-item.component.html',
  styleUrls: ['./update-table-item.component.scss'],
})

export class UpdateTableItemComponent extends Modal implements OnInit {

  TITLE_HEADER_FORM = 'Actualizar Registro table_items';
  ...
  tableItem: TableItem;
  modalInput(tableItem: TableItem): void {
    this.tableItem = tableItem;
  }

  ngOnInit(): void {
    this.getTableItemService(TableMasterEnum.ESTADO_DE_REGISTRO);
    this.getTableItem(this.tableItem);
  }
  ...
  getTableItem(tableItem: TableItem) {
    this.tableItemService.getTableItem(tableItem.id).subscribe(
      (resultGetTableItem) => {
        this.tableItem = TableItem.createTableItem(resultGetTableItem);
      },
      (errorArray) => {
  ...
  updateTableItem() {
    this.tableItemService
      .putTableItem(this.tableItem.toDataPersistJson())
      .subscribe(
        (resultPut) => {
          this.alertService.openAlertInfo(resultPut);
          this.modalClose(this.tableItem);
        },
        (errorArray) => {
          this.tableItem.errors = [errorArray];
        }
      );
  }

  validateSaveRegister() {
    if (this.tableItem.isValid) {
      this.dialogService
        .openDialog({
