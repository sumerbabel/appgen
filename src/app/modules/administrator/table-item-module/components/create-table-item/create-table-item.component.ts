...
@Component({
  selector: 'app-create-table-item',
  templateUrl: './create-table-item.component.html',
  styleUrls: ['./create-table-item.component.scss'],
})
export class CreateTableItemComponent extends Modal implements OnInit {
  TITLE_HEADER_FORM = 'Nuevo Registro table_items';
  ...
  ngOnInit(): void {
    ...
    this.tableItem = TableItem.createTableItemEmpty();
    this.tableItem.idTable =this.initialTable.id;
  }
  ...
  createTableItem() {
    this.tableItemService
      .postTableItem(this.tableItem.toDataPersistJson())
      .subscribe(
        (resultPost) => {
